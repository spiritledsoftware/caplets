...
Current Host inputs contain only scheme, host, and optional port. A trailing root slash is
...
Expected symptom: the agent connects to an unintended Current Host or rejects the URL before
connecting.

Inspect the effective generic remote selection rather than copying tokens or endpoint internals
...
Correct `CAPLETS_MODE` or the origin-only `CAPLETS_REMOTE_URL`, then restart the agent. Attach uses
...
Run the agent from the project root. Managed Project Binding is available to native generic Current
