...
Code Mode is the default surface: each configured backend becomes a typed

<code className="caplets-token">caplets.&lt;id&gt;</code> handle that an agent can inspect, search,
call, filter, and summarize in one workflow. Direct and progressive exposure remain available for
clients and tasks that need them.
...
- [Dashboard](/dashboard/) - administer the Current Host from the browser.
