...
If you start or expose an HTTP runtime, the same Current Host serves the browser administration
...
Current Host protocol namespaces are origin-root paths. A reverse proxy must expose them directly;
