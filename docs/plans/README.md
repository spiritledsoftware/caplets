...
Admin API decision session: 2026-07-20  
...
Plan 000 is the executable Admin API migration anchor, not the first code change. Complete its listed containment, database, Vault, OAuth, and dashboard-characterization prerequisites before starting it. Plans that depend on 000 assume ADR 0007's resource shape and generated contract.
...
Plans omitted from the graph are independent at the source-contract level. They may still conflict mechanically. In particular 001/005 touch HTTP composition, 003/004/007/012 touch authoritative storage and concurrency, and 000/009/010/011/013/019 touch Admin/dashboard/Caplet Record seams. Plan 020 is the final public-client completion slice on the Plan 000 branch: it consumes Plan 000's canonical contract, and Plan 000 remains in progress until 020 is green. Land prerequisites separately, rebase Plan 000 after them, and rebase dependent plans after 000.
...
### Wave B — Admin API prerequisites and migration

1. Execute 004 and 007 after 003 is green in CI.
2. Execute 000 only after 001, 003, 004, 007, and 009 are complete.
3. Keep 000 as one end-to-end migration branch with internally reviewable steps; it is not releasable until both Admin mounts, root OpenAPI, generated clients, streaming bundles, frozen v1 compatibility, and smoke scenarios pass.
4. Execute 020 on the Plan 000 branch after the canonical OpenAPI contract is present and before final review, changeset validation, or commit.

### Wave C — post-migration deepening
...
| ---------------------------------------- | ---: | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Resource-oriented Current Host Admin API |  000 | ADR 0007: one semantic Module, distinct bearer/dashboard Adapters, root non-MCP OpenAPI, HeyAPI clients, streaming bundles, frozen v1 compatibility. |
...
- **Mixed-auth Admin routes:** ADR 0007 keeps bearer and dashboard cookie/CSRF as distinct Adapters over the same relative resource handlers.
- **Permanent dual v1/v2 evolution:** v1 remains available but frozen; new capabilities go only to v2.
...
- **Hosted Cloud control-plane work:** separately owned and outside this repository audit's verified runtime scope.
