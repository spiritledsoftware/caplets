...
- P-R13. Default stdio `serve` and `attach` suppress passive notices by default, even when stderr is a TTY; `attach` becomes eligible only through explicit opt-in, while `serve` follows its transport-based eligibility rules. Origin refs: R11, R12, F2, AE4, AE5.
- P-R14. Foreground HTTP or SSE `serve` may be eligible when stderr is human-facing, but daemon-managed service processes and native integrations are not passive notice channels and must not evaluate update cache or registry paths in v1. Origin refs: R6, R11, R13.
