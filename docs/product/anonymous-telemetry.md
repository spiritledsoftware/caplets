...
Public-site analytics use the same anonymous boundary. Landing, docs, and catalog events use categorical route, section, search, filter, and install-intent fields. Browser analytics disable session replay, heatmaps, broad autocapture, raw DOM capture, persistence, and known-user identification for this pass.
