...
export async function migrateHostDatabase(database: HostDatabase): Promise<void> {
  ...
  if (database.dialect === "sqlite") {
    await migrateSqliteExclusively(database, appliedAt);
    return;
  }
...
async function migrateSqliteExclusively(
  ...
  try {
    ...
    ).rows[0] as { present?: number } | undefined;
    if (schemaTable?.present === 1) {
      const row = (
        await transaction.execute("SELECT version FROM caplets_schema WHERE singleton = 1")
    ...
    const appliedMigrations = (
      await transaction.execute(
        "SELECT hash, created_at FROM caplets_migrations ORDER BY created_at",
      )
    ...
    for (const migration of migrations) {
      ...
      for (const statement of migration.sql) await transaction.executeMultiple(statement);
      await transaction.execute({
        sql: "INSERT INTO caplets_migrations (hash, created_at) VALUES (?, ?)",
        args: [migration.hash, migration.folderMillis],
      });
    }
    await transaction.execute({
      sql: "INSERT INTO caplets_schema (singleton, version, applied_at) VALUES (1, ?, ?) ON CONFLICT(singleton) DO UPDATE SET version = excluded.version, applied_at = excluded.applied_at",
      args: [HOST_STORAGE_SCHEMA_VERSION, appliedAt],
    });
