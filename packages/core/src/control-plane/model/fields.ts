...
> = {
  ...
  "artifact-session": [
    ...
    text("state"),
    version("nextOffset"),
    version("expectedByteLength"),
    field("expectedSha256", "hash"),
    text("mimeType"),
    field("providerIdentityId", "hash"),
    clock("expiresAt"),
    clock("finalizedAt", false),
  ...
  "artifact-quota-reservation": [
    ...
    id("actorId"),
    id("sessionId"),
    version("reservedBytes"),
    text("state"),
    clock("windowExpiresAt"),
    clock("releasedAt", false),
  ],
  "import-proposal": [
    id("proposalId"),
    id("artifactId"),
    ...
    id("operationId"),
    id("capletId"),
    field("proposalHash", "hash"),
    version("expectedAuthorityGeneration"),
    version("expectedEffectiveGeneration"),
    version("expectedAggregateVersion"),
    version("expectedSecurityEpoch"),
    field("expectedRuntimeFingerprint", "hash"),
    text("collisionPolicy"),
    field("replacementConfirmed", "boolean"),
    text("consequence"),
    json("differences", true),
    json("setupDependencies", true),
    text("state"),
    clock("expiresAt"),
    clock("consumedAt", false),
  ],
