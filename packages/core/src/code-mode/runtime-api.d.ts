...
type BodyInit = Blob | FormData | string | ArrayBuffer | ArrayBufferView | null | undefined;
...
interface BufferConstructor {
  from(
    input: string | ArrayLike<number> | ArrayBuffer | ArrayBufferView | Buffer,
...
declare class Blob {
  constructor(parts?: BlobPart[], options?: { type?: string; endings?: EndingType });
...
declare class AbortSignal {
  ...
  addEventListener(type: "abort", listener: AbortListener | null): void;
  removeEventListener(type: "abort", listener: AbortListener | null): void;
...
/**
 * Direct network fetch is unavailable in Code Mode. Use a Caplet handle instead.
 * Static diagnostics report FETCH_UNAVAILABLE for calls to this function.
 */
...
declare function setTimeout(callback: TimerHandler, delay?: number, ...args: unknown[]): number;
declare function clearTimeout(timerId: number): void;
declare function setInterval(callback: TimerHandler, delay?: number, ...args: unknown[]): number;
