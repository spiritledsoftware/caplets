...
CREATE TABLE `__new_cp_host_setting` (
	...
	CONSTRAINT "cp_host_setting_typed_value_check" CHECK((
          (
            "__new_cp_host_setting"."key" = 'native.daemon-url'
            AND json_type("__new_cp_host_setting"."value", '$') = 'object'
            AND json_extract("__new_cp_host_setting"."value", '$.source') = 'setup'
            AND json_type("__new_cp_host_setting"."value", '$.url') = 'text'
            AND json_remove("__new_cp_host_setting"."value", '$.source', '$.url') = '{}'
          )
          OR ("__new_cp_host_setting"."key" = 'telemetry' AND json_type("__new_cp_host_setting"."value", '$') IN ('true', 'false'))
          OR (
            "__new_cp_host_setting"."key" IN (
            ...
            AND json_type("__new_cp_host_setting"."value", '$') = 'integer'
            AND json_extract("__new_cp_host_setting"."value", '$') > 0
          )
          OR (
            "__new_cp_host_setting"."key" = 'options.maxSearchLimit'
            AND json_type("__new_cp_host_setting"."value", '$') = 'integer'
            AND json_extract("__new_cp_host_setting"."value", '$') BETWEEN 1 AND 50
          )
          OR (
            "__new_cp_host_setting"."key" = 'options.exposureDiscoveryConcurrency'
            AND json_type("__new_cp_host_setting"."value", '$') = 'integer'
            AND json_extract("__new_cp_host_setting"."value", '$') BETWEEN 1 AND 32
          )
          OR (
            "__new_cp_host_setting"."key" IN (
          ...
          OR (
            "__new_cp_host_setting"."key" = 'options.exposure'
            AND json_type("__new_cp_host_setting"."value", '$') = 'text'
            AND json_extract("__new_cp_host_setting"."value", '$') IN (
              'direct',
              'progressive',
              'code_mode',
          ...
          OR (
            "__new_cp_host_setting"."key" = 'namespaceAliases'
            AND json_type("__new_cp_host_setting"."value", '$') = 'object'
            AND json_type("__new_cp_host_setting"."value", '$.upstreams') = 'object'
            AND json_remove("__new_cp_host_setting"."value", '$.local', '$.upstreams') = '{}'
            AND (
              json_type("__new_cp_host_setting"."value", '$.local') IS NULL
              OR (
                json_type("__new_cp_host_setting"."value", '$.local') = 'text'
                AND length(json_extract("__new_cp_host_setting"."value", '$.local')) BETWEEN 1 AND 32
                AND substr(json_extract("__new_cp_host_setting"."value", '$.local'), 1, 1) GLOB '[a-z]'
                AND NOT json_extract("__new_cp_host_setting"."value", '$.local') GLOB '*[^a-z0-9-]*'
                AND (
                  length(json_extract("__new_cp_host_setting"."value", '$.local')) = 1
                  OR substr(json_extract("__new_cp_host_setting"."value", '$.local'), -1, 1) GLOB '[a-z0-9]'
                )
              )
            )
            AND json_extract("__new_cp_host_setting"."value", '$.upstreams') NOT GLOB '*:[^"]*'
            AND json_extract("__new_cp_host_setting"."value", '$.upstreams') NOT GLOB '*:""*[},]'
            AND json_extract("__new_cp_host_setting"."value", '$.upstreams') NOT GLOB '*:"[^a-z]*'
            AND json_extract("__new_cp_host_setting"."value", '$.upstreams') NOT GLOB '*-"[},]*'
          )
        ))
);
